...
cases_train['age'] = cases_train['age'].apply(
    lambda x: helper1.range_to_num(x))
...
cases_train[['sex', 'date_confirmation', 'additional_information', 'source']] = cases_train[[
    'sex', 'date_confirmation', 'additional_information', 'source']].fillna('unknown')
cases_test[['sex', 'date_confirmation', 'additional_information', 'source']] = cases_test[[
    'sex', 'date_confirmation', 'additional_information', 'source']].fillna('unknown')

# Clean the locations data
cases_location.dropna(
    subset=['Incident_Rate', 'Case_Fatality_Ratio'], inplace=True)
...
cases_location = cases_location[~((cases_location['Confirmed'] > 1000) & (
    cases_location['Case_Fatality_Ratio'] < 0.1))].reset_index(drop=True)


# 1.6 Joining the cases and location dataset
cases_location['Country_Region'] = cases_location['Country_Region'].apply(
    helper1.fix_country_label)

# Group the location data by province and country
cases_location['Province_State'].fillna('', inplace=True)
cases_location['Population'] = cases_location['Confirmed'] * \
    100000 / cases_location['Incident_Rate']
...
grouped_locations['Incident_Rate'] = grouped_locations['Confirmed'] / \
    grouped_locations['Population'] * 100000
grouped_locations['Case_Fatality_Ratio'] = grouped_locations['Deaths'] / \
    grouped_locations['Confirmed'] * 100
...
# join datasets
combined_train = pd.merge(cases_train, grouped_locations, how='inner', left_on=[
                          'country', 'province'], right_on=['Country_Region', 'Province_State'])
combined_test = pd.merge(cases_test, grouped_locations, how='inner', left_on=[
                         'country', 'province'], right_on=['Country_Region', 'Province_State'])
...
train_features = combined_train[['age', 'sex', 'province', 'country', 'date_confirmation', 'chronic_disease_binary',
                                 'confirmed', 'deaths', 'population', 'incident_rate', 'case_fatality_ratio', 'outcome_group']]
test_features = combined_test[['age', 'sex', 'province', 'country', 'date_confirmation', 'chronic_disease_binary',
                               'confirmed', 'deaths', 'population', 'incident_rate', 'case_fatality_ratio', 'outcome_group']]
